...
import { 
  ...
  Phone, Video, MoreVertical, CheckCheck, TrendingUp
} from 'lucide-react';
...
const PROPRIETAIRES_PAR_DEFAUT = [
  { id: 'prop-1', name: 'M. Koné', phone: '07 08 09 10 11', company: 'Koné Transports' },
  { id: 'prop-2', name: 'Mme Bamba', phone: '05 02 03 04 05', company: 'Bamba Star' }
];
...
const INITIAL_FORM_STATE = {
  ...
  vehicles: [
    ...
    { id: 2, name: 'Suzuki bleu', amount: '', reason: 'RAS (Journée normale)', customReason: '' }
  ],
  expenses: [],
  arrears: {
    previous: '',
...
export default function App() {
  ...
  const [previewMode, setPreviewMode] = useState('whatsapp'); // 'whatsapp' | 'fiche'
  ...
  // 1. Initialisation Auth (Connexion Anonyme pour garder le même fonctionnement)
  useEffect(() => {
    const initAuth = async () => {
      try {
        ...
        if (error.code === 'auth/configuration-not-found' || error.code === 'auth/operation-not-allowed') {
          setFirebaseErrorMsg("Activez l'authentification dans Firebase : Console > Authentication > Onglet Sign-in method > Fournisseur 'Anonyme'.");
        } else {
          showToast("⚠️ Problème de connexion Firebase");
        }
      }
    };
    initAuth();
  ...
  useEffect(() => {
    if (!user) return;
    // Chemin standard pour un vrai projet Firebase: users/{uid}/reports
  ...
  const saveToHistory = async () => {
    if (!user) {
      showToast('⚠️ Mode Local (Firebase non connecté)');
      const dataToSave: any = { ...formData, id: editingId || Date.now().toString(), updatedAt: new Date().toISOString() };
      if (!editingId) dataToSave.createdAt = new Date().toISOString();
      
      if (editingId) {
        setHistory(prev => prev.map(item => item.id === editingId ? dataToSave : item));
        showToast('📌 Rapport mis à jour (Localement)');
      } else {
        setHistory(prev => [dataToSave, ...prev].sort((a, b) => new Date(b.reportDate).valueOf() - new Date(a.reportDate).valueOf()));
        showToast('📌 Nouveau rapport enregistré (Localement)');
        resetForm();
      }
  ...
  const loadReport = (item) => {
    ...
    setFormData({
      ...
      ownerId: item.ownerId || owners[0]?.id || 'prop-1',
      vehicles: item.vehicles || [],
      expenses: item.expenses || [],
      arrears: item.arrears || { previous: '', paid: '', reason: '', proofName: '' },
      alerts: item.alerts || [],
      customProblem: item.customProblem || ''
    });
    setActiveTab('form');
    showToast('✏️ Rapport chargé dans l\'éditeur');
  };
  ...
  const handleSimulateAttachment = (e) => {
    e.preventDefault();
    const fakeFileName = `recu_${Math.floor(Math.random() * 90000) + 10000}.pdf`;
    updateArrears('proofName', fakeFileName);
    showToast(`📎 Fichier attaché : ${fakeFileName}`);
  };
  ...
  const arrearsCalculated = useMemo(() => {
    ...
    const totalDetteDûe = previous + totalExpenses;
    const resteDette = totalDetteDûe - paid;
    return {
      totalDetteDûe,
      resteDette
    };
  }, [formData.arrears.previous, formData.arrears.paid, totalExpenses]);

  const activeOwnerObj = useMemo(() => {
    return owners.find(o => o.id === formData.ownerId) || owners[0];
  ...
  const generateWhatsAppText = () => {
    ...
    text += `🏢 *Propriété :* ${owner?.company || 'Non spécifié'} (${owner?.name})\n`;
    text += `📅 *Date :* ${formattedDate}\n`;
    text += `───────────────────\n\n`;

    text += `💰 *1. DEPOT RECETTE PAR VEHICULE*\n\n`;
    ...
    text += `\n🧾 *Suivi Arriérés & Créances :*\n`;
    text += `• Ancien arriéré : ${formatAmount(d.arrears.previous)} FCFA\n`;
    text += `• Nouvelle dette cumulée : ${formatAmount(arrearsCalculated.totalDetteDûe)} FCFA\n`;
    text += `• Payé ce jour : *${formatAmount(d.arrears.paid)} FCFA*\n`;
    text += `• 📌 *Reste de la dette :* *${formatAmount(arrearsCalculated.resteDette)} FCFA*\n`;
    
    if (d.arrears.reason) {
      text += `_Justification paiement : ${d.arrears.reason}_\n`;
    }
    if (d.arrears.proofName) {
      text += `📎 _Preuve attachée : ${d.arrears.proofName}_\n`;
    }
    text += `───────────────────\n\n`;

    if (d.alerts.length > 0 || d.customProblem.trim()) {
      text += `⚠️ *3. ALERTES & ALERTES ADMINISTRATIVE*\n\n`;
      d.alerts.forEach(a => {
      ...
      text += `✨ *3. ALERTES :* RAS (Aucune alerte)\n`;
    }
  ...
  const handleDownloadImage = () => {
    ...
    showToast(`📸 Génération de l'image (${previewMode === 'whatsapp' ? 'Simulateur WA' : 'Fiche Pro'})...`);

    const generateImage = (el: HTMLElement) => {
      (window as any).html2canvas(el, { 
        ...
        link.download = `rapport-${previewMode}-${formData.reportType.toLowerCase()}-${formData.reportDate}.png`;
  ...
  const handleAddVehicleOption = (e) => {
    e.preventDefault();
    ...
    showToast('🚗 Véhicule enregistré dans vos configurations');
  };
  ...
  const renderFormattedBubbleText = (rawText) => {
    ...
    return lines.map((line, i) => {
      ...
      } else if (line.includes('💰') || line.includes('TOTAL RECETTE') || line.includes('💵')) {
        textClass = "text-emerald-700 font-bold";
        containerClass += " mt-1";
      } else if (line.includes('💸') || line.includes('Total Dépenses') || line.includes('🔻') || line.includes('DEPENSES & COMPTES')) {
        textClass = "text-rose-700 font-bold";
        containerClass += " mt-1";
      } else if (line.includes('🧾') || line.includes('Reste de la dette') || line.includes('📌') || line.includes('Suivi Arriérés')) {
      ...
      const htmlContent = formatMarkdown(line);

      return (
        <div key={i} className={containerClass}>
          <span className={textClass} dangerouslySetInnerHTML={{ __html: htmlContent }} />
        </div>
      );
    });
  };
  ...
  return (
    <div className="min-h-screen bg-[#faf8f5] font-sans text-slate-800 pb-20 lg:pb-8 flex flex-col">
      ...
      {/* --- BANNIÈRE ERREUR FIREBASE --- */}
      {firebaseErrorMsg && (
        <div className="bg-red-600 text-white px-4 py-3 text-sm font-bold text-center flex items-center justify-center gap-2 z-50 shadow-md">
      ...
      {/* --- TOAST --- */}
      {toast && (
        <div className="fixed top-6 left-1/2 transform -translate-x-1/2 bg-amber-900 text-yellow-100 px-6 py-3 rounded-full shadow-2xl z-50 font-medium animate-bounce border-2 border-amber-600 flex items-center gap-2 text-sm">
      ...
      {itemToDelete && (
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-[#fef9c3] rounded-2xl shadow-2xl max-w-sm w-full p-6 border-4 border-amber-400 rotate-1 relative">
            ...
            <h3 className="text-xl font-bold text-center text-amber-950 mb-2">Supprimer ce Post-it ?</h3>
            <p className="text-amber-800 text-center text-sm mb-6">Cette action détruira définitivement ce rapport d'activité.</p>
      ...
      {isOwnerModalOpen && (
        <div className="fixed inset-0 bg-slate-900/50 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-[#eff6ff] rounded-2xl shadow-xl max-w-md w-full p-6 border-4 border-blue-400 relative">
            ...
            <div className="flex justify-between items-center mb-4 border-b-2 border-dashed border-blue-200 pb-2">
              <h3 className="text-lg font-bold text-blue-900 flex items-center gap-2"><UserPlus size={20}/> Ajouter un Propriétaire</h3>
              <button onClick={() => setIsOwnerModalOpen(false)} className="text-blue-500 hover:text-blue-700"><X size={20}/></button>
            </div>
            <form onSubmit={handleAddOwner} className="space-y-4">
              ...
              <div>
                <label className="block text-xs font-bold text-blue-800 uppercase tracking-wider mb-1">Entreprise / Nom de Flotte</label>
                <input type="text" placeholder="Ex: Koné Transports" value={newOwner.company} onChange={e => setNewOwner({...newOwner, company: e.target.value})} className="w-full p-2.5 bg-white border-2 border-blue-200 rounded-xl focus:border-blue-500 outline-none text-sm" />
              </div>
              <button type="submit" className="w-full py-3 bg-blue-600 hover:bg-blue-700 text-white rounded-xl font-bold text-sm shadow-md transition-all">Enregistrer le Propriétaire</button>
            </form>
          </div>
        </div>
      )}

      {/* --- MODAL AJOUT OPTION VÉHICULE --- */}
      {isVehicleModalOpen && (
        <div className="fixed inset-0 bg-slate-900/50 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-[#fdf2f8] rounded-2xl shadow-xl max-w-sm w-full p-6 border-4 border-pink-400 relative">
            ...
            <div className="flex justify-between items-center mb-4 border-b-2 border-dashed border-pink-200 pb-2">
              <h3 className="text-lg font-bold text-pink-900 flex items-center gap-2"><Car size={20}/> Nouveau Véhicule</h3>
              <button onClick={() => setIsVehicleModalOpen(false)} className="text-pink-500 hover:text-pink-700"><X size={20}/></button>
            </div>
            <form onSubmit={handleAddVehicleOption} className="space-y-4">
              <div>
                <label className="block text-xs font-bold text-pink-800 uppercase tracking-wider mb-1">Nom ou Immatriculation</label>
              ...
              <button type="submit" className="w-full py-3 bg-pink-600 hover:bg-pink-700 text-white rounded-xl font-bold text-sm shadow-md transition-all">Ajouter à ma liste de flotte</button>
            </form>
          </div>
        </div>
      )}
      ...
      <header className="bg-gradient-to-r from-amber-700 to-amber-900 text-white shadow-lg sticky top-0 z-40">
        <div className="max-w-7xl mx-auto px-4 py-3.5 flex justify-between items-center">
          <h1 className="text-xl font-bold flex items-center gap-2">
            ...
            Rapport <span className="text-amber-200 font-normal">Véhicule</span>
          </h1>
      ...
      <main className="max-w-7xl mx-auto p-4 sm:p-6 w-full flex-1 flex flex-col lg:flex-row gap-8">
        ...
        <div className={`w-full lg:w-[55%] xl:w-[60%] flex flex-col gap-6 ${activeTab !== 'form' && activeTab !== 'preview' ? 'lg:hidden hidden' : (activeTab === 'preview' ? 'hidden lg:flex' : 'flex')}`}>
            ...
            <div className="space-y-8 animate-in fade-in duration-300">
              ...
              <div className="bg-[#eff6ff] p-6 rounded-3xl border-2 border-blue-200 relative shadow-[5px_5px_0px_0px_rgba(191,219,254,1)] transform hover:rotate-1 transition-transform">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  ...
                  <div>
                    <label className="block text-[11px] font-bold text-blue-700 uppercase tracking-wider mb-1">Date du rapport</label>
              ...
              {/* STICKY NOTE : 1. RECETTES PAR VÉHICULE */}
              <div className="bg-[#fef9c3] p-6 rounded-3xl border-2 border-yellow-200 relative shadow-[5px_5px_0px_0px_rgba(254,240,138,1)] transform hover:-rotate-1 transition-transform">
                ...
                <div className="flex justify-between items-center mb-4 border-b-2 border-dashed border-yellow-300 pb-3">
                  ...
                  <div className="flex items-center gap-2">
                    <span className="text-xs font-bold text-yellow-800 bg-yellow-200/50 px-2 py-1 rounded-lg">Total Recette : {formatAmount(totalRecette)} FCFA</span>
                    <button type="button" onClick={() => setIsVehicleModalOpen(true)} className="text-xs bg-yellow-200/50 hover:bg-yellow-200 text-yellow-900 font-bold p-1 rounded-lg"><Plus size={14}/></button>
                  </div>
                </div>
                  ...
                  {formData.vehicles.map((v, index) => (
                    <div key={v.id} className="p-4 bg-white/75 rounded-2xl border border-yellow-200 shadow-sm relative group">
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-3 pr-6">
                        ...
                        <div>
                          <label className="text-[10px] uppercase font-bold text-yellow-800 mb-0.5 block">Montant versé (FCFA)</label>
              ...
              <div className="bg-[#dcfce7] p-6 rounded-3xl border-2 border-green-200 relative shadow-[5px_5px_0px_0px_rgba(187,247,208,1)] transform hover:rotate-1 transition-transform">
                ...
                <div className="flex justify-between items-center mb-4 border-b-2 border-dashed border-green-300 pb-3">
                  ...
                  <span className="text-xs font-bold text-green-800 bg-green-200/50 px-2 py-1 rounded-lg">Total Dépenses : {formatAmount(totalExpenses)} FCFA</span>
                </div>
              ...
              <div className="bg-[#f3e8ff] p-6 rounded-3xl border-2 border-purple-200 relative shadow-[5px_5px_0px_0px_rgba(233,213,255,1)] transform hover:-rotate-1 transition-transform">
                ...
                <div className="flex justify-between items-center mb-4 border-b-2 border-dashed border-purple-300 pb-3">
                  <h3 className="font-bold text-purple-950 flex items-center gap-2"><Clipboard size={18}/> Suivi des Arriérés</h3>
                  <span className="text-xs font-bold text-purple-800 bg-purple-200/50 px-2 py-1 rounded-lg">Reste : {formatAmount(arrearsCalculated.resteDette)} FCFA</span>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                  ...
                  <div>
                    <label className="text-[10px] uppercase font-bold text-purple-800 mb-0.5 block">Payé ce jour (Remboursement)</label>
                    <div className="relative">
                      <input type="number" placeholder="0" value={formData.arrears.paid} onChange={e => updateArrears('paid', e.target.value)} className="w-full p-2.5 bg-white border-2 border-purple-200 rounded-xl focus:border-purple-500 outline-none text-sm text-purple-900 pr-12 font-bold" />
                      <span className="absolute right-3 top-1/2 -translate-y-1/2 text-[10px] font-bold text-purple-600">FCFA</span>
                    </div>
                  </div>
                </div>
                ...
                <div className="space-y-3">
                  <div>
                    <label className="text-[10px] uppercase font-bold text-purple-800 mb-0.5 block">Justification du paiement (Facultatif)</label>
                    <input type="text" placeholder="Ex: Virement Wave, Remise en main propre..." value={formData.arrears.reason} onChange={e => updateArrears('reason', e.target.value)} className="w-full p-2.5 bg-white border-2 border-purple-200 rounded-xl focus:border-purple-500 outline-none text-sm text-purple-900" />
                  </div>
                  
                  <div className="flex items-center gap-3 mt-2">
                    <button onClick={handleSimulateAttachment} className="flex-1 py-2 bg-purple-100 hover:bg-purple-200 text-purple-700 font-bold rounded-xl border border-purple-300 text-xs flex justify-center items-center gap-2 transition-all">
                      <Paperclip size={14}/> Joindre Reçu (Simulé)
                    </button>
                    {formData.arrears.proofName && (
                      <span className="flex-1 text-[11px] text-purple-600 truncate bg-purple-50 p-2 rounded-lg border border-purple-100">
                        📎 {formData.arrears.proofName}
                      </span>
                    )}
                  </div>
              ...
              <div className="bg-[#ffedd5] p-6 rounded-3xl border-2 border-orange-200 relative shadow-[5px_5px_0px_0px_rgba(253,186,116,1)] transform hover:rotate-1 transition-transform">
                ...
                <div className="flex justify-between items-center mb-4 border-b-2 border-dashed border-orange-300 pb-3">
                  <h3 className="font-bold text-orange-950 flex items-center gap-2"><ShieldAlert size={18}/> 3. Alertes & Remarques</h3>
                </div>
              ...
              <div className="flex gap-3 pt-4 sticky bottom-4 z-10 lg:static lg:bg-transparent bg-[#faf8f5]/90 backdrop-blur-sm p-4 lg:p-0 rounded-2xl">
          ...
          {/* HISTORIQUE (Quand affiché côté gauche ou plein écran sur mobile) */}
          {activeTab === 'history' && (
            <div className="space-y-6 animate-in fade-in duration-300">
              <div className="bg-white p-5 rounded-3xl border border-slate-200 shadow-sm flex flex-col sm:flex-row gap-4 justify-between items-center">
                <h2 className="text-xl font-bold text-slate-800 flex items-center gap-2">
                  <History className="text-amber-600" /> Archives de rapports
                </h2>
                      ...
                      <div key={item.id} className="bg-white p-5 rounded-3xl border-2 border-slate-100 shadow-sm hover:shadow-md transition-all relative overflow-hidden group">
                        ...
                        <div className="relative z-10">
                          ...
                          <div className="flex gap-4 text-sm text-slate-600 mb-4 border-y border-slate-100 py-3">
                            ...
                            <div>
                              <span className="block text-[10px] uppercase text-slate-400 font-bold">Alertes</span>
                              <span className="font-bold text-orange-500">{item.alerts?.length || 0}</span>
                            </div>
                          </div>
        ...
        {/* === RIGHT COLUMN: PREVIEW (DOUBLED MODE - PROFESSIONAL DESIGN) === */}
        <div className={`w-full lg:w-[45%] xl:w-[40%] flex-col gap-4 ${activeTab === 'preview' ? 'flex' : 'hidden lg:flex'}`}>
          <div className="bg-white rounded-3xl shadow-xl border border-slate-200 overflow-hidden flex flex-col h-[calc(100vh-120px)] sticky top-24">
            
            {/* Nav Mode de Preview */}
            <div className="bg-slate-100 p-2 flex border-b border-slate-200 gap-1">
              <button 
              ...
              >
                <FileText size={15}/> 💬 Format Message
              </button>
              <button 
            ...
            <div className="bg-white border-b border-slate-100 p-4 flex justify-between items-center">
              <div>
                ...
                <p className="text-[10px] text-slate-400">Prêt pour transfert ou archivage</p>
              </div>
              <div className="flex gap-1.5">
                 ...
                 <button onClick={handleDownloadImage} disabled={isGeneratingImg} className="p-2.5 bg-indigo-600 hover:bg-indigo-700 text-white rounded-xl shadow-md transition-all tooltip flex items-center gap-1.5 text-xs font-bold" title="Télécharger l'image de l'aperçu">
            ...
            <div className="flex-1 overflow-y-auto p-4 bg-slate-50 flex flex-col items-center">
              
              {/* MODE 1: FORMAT MESSAGE (RECAPITULATIF SUR FOND VERT) */}
              {previewMode === 'whatsapp' && (
                <div 
                  id="report-whatsapp-container" 
                  className="w-full max-w-md rounded-3xl shadow-sm border border-emerald-200 overflow-hidden bg-[#e1fcdc] p-6 flex flex-col transform transition-transform"
                >
                  {/* Texte brut avec couleurs de différenciation direct sur le fond */}
              ...
              {/* MODE 2: PROFESSIONAL BUSINESS CARD RECEIPT VIEW */}
              {previewMode === 'fiche' && (
                <div 
                  id="report-fiche-container" 
                  className="w-full max-w-sm bg-white rounded-3xl shadow-lg border border-slate-200/80 overflow-hidden flex flex-col p-6 text-slate-800"
                >
                  {/* Decorative bar */}
                  <div className="h-2 bg-gradient-to-r from-amber-500 via-indigo-500 to-emerald-500 -mx-6 -mt-6 mb-6"></div>
                  ...
                  <div className="space-y-3 mb-6">
                    <h5 className="text-[10px] font-bold text-slate-400 uppercase tracking-widest flex items-center gap-1">
                      <Wallet size={12} className="text-amber-500"/> 1. Recettes véhicules ({formData.vehicles.length})
                    </h5>
                      ...
                      {formData.vehicles.map((v) => {
                        ...
                        return (
                          <div key={v.id} className="bg-slate-50 p-2.5 rounded-xl border border-slate-100 flex justify-between items-center text-xs">
                            <div>
                              <span className="font-bold text-slate-700 block">{vName}</span>
                              {hasIncident && (
                                <span className="text-[9px] text-amber-600 bg-amber-50 border border-amber-100 px-1.5 py-0.2 rounded-md font-medium mt-0.5 inline-block">
                  ...
                  <div className="space-y-3 mb-6">
                    <h5 className="text-[10px] font-bold text-slate-400 uppercase tracking-widest flex items-center gap-1">
                      <DollarSign size={12} className="text-emerald-500"/> 2. Dépenses enregistrées
                    </h5>
                        ...
                        {formData.expenses.map((e) => {
                          ...
                          return (
                            <div key={e.id} className="bg-red-50/40 p-2 rounded-xl border border-red-100/50 flex justify-between items-center text-[11px]">
                              <span className="text-slate-600 font-medium truncate max-w-[200px]">{e.vehicleName} - {desc}</span>
                              <span className="font-bold text-red-600">- {formatAmount(e.amount)} F</span>
                            </div>
                          );
                        })}
                        <div className="flex justify-between items-center text-xs font-bold text-red-700 pt-1">
                          <span>Total Dépenses :</span>
                          <span>{formatAmount(totalExpenses)} FCFA</span>
                        </div>
                      </div>
                    ) : (
                      <p className="text-[11px] text-slate-400 italic pl-1">Aucune dépense signalée ce jour.</p>
                    )}
                  </div>
                  ...
                  <div className="space-y-3 mb-6 bg-indigo-50/50 border border-indigo-100 p-4 rounded-2xl">
                    <h5 className="text-[10px] font-bold text-indigo-900 uppercase tracking-widest flex items-center gap-1">
                      <Clipboard size={12} className="text-indigo-600"/> 3. Suivi Comptabilité & Solde
                    </h5>
                    
                    <div className="grid grid-cols-2 gap-2 text-xs">
                      <div className="bg-white p-2 rounded-xl border border-indigo-100">
                        <span className="block text-[9px] text-slate-400 font-medium">Dette d'hier</span>
                        <span className="font-bold text-slate-700">{formatAmount(formData.arrears.previous)} F</span>
                      </div>
                      <div className="bg-white p-2 rounded-xl border border-indigo-100">
                        <span className="block text-[9px] text-slate-400 font-medium">Cumul + Dépenses</span>
                        <span className="font-bold text-slate-700">{formatAmount(arrearsCalculated.totalDetteDûe)} F</span>
                      </div>
                      <div className="bg-emerald-50/50 p-2 rounded-xl border border-emerald-100 col-span-2 flex justify-between items-center">
                        <div>
                          <span className="block text-[9px] text-emerald-800 font-bold uppercase">Remboursé ce Jour</span>
                          {formData.arrears.reason && <span className="text-[8px] text-emerald-600">{formData.arrears.reason}</span>}
                        </div>
                        <span className="font-bold text-emerald-700 text-sm">{formatAmount(formData.arrears.paid)} FCFA</span>
                      </div>
                    </div>

                    <div className="border-t border-indigo-100 pt-2.5 flex justify-between items-center text-xs">
                  ...
                  {(formData.alerts.length > 0 || formData.customProblem.trim()) && (
                    <div className="space-y-2 mb-4">
                      ...
                      <div className="space-y-1.5">
                        {formData.alerts.map((a) => (
                          <div key={a.id} className="text-[11px] bg-orange-50 text-orange-800 p-2 rounded-xl border border-orange-100">
                            <strong>{a.vehicleName}</strong> : {a.category} - {a.description || 'Problème'}
                          </div>
                        ))}
                        {formData.customProblem.trim() && (
                          <div className="text-[11px] bg-slate-50 text-slate-600 p-2 rounded-xl border border-slate-200 italic">
